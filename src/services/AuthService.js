...
export class AuthService {
  checkUsernameEmail = async (username, email) => {
    try {
      const response = await fetch(
        `https://x2025unbored786979363000.francecentral.cloudapp.azure.com/auth/otp`,
        {
          method: "POST",
          headers: {
  ...
  checkOTP = async (email, otp) => {
    try {
      const response = await fetch(
        `https://x2025unbored786979363000.francecentral.cloudapp.azure.com/auth/otp/verify`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  loginGoogle = async (googleTokenId) => {
    try {
      const response = await fetch(
        `https://x2025unbored786979363000.francecentral.cloudapp.azure.com/auth/login/google`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",

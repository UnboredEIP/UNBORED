...
const Activities = ({ navigation }) => {
  ...
  return (
    <View style={styles.container}>
        ...
        {data && data.results.map((character, index) => (
          ...
          >
            <View style={[styles.card, swipeDirection === 'right' ? styles.swipeRight : swipeDirection === 'left' ? styles.swipeLeft : null]}>
              <ImageBackground style={styles.cardImage} source={{ uri: character.cover_url }}>
      ...
      {currentCardIndex === -1 && <Text style={styles.infoText}>Félicitation ! Vous avez tout swipé !</Text>}
      {data && data.results[currentCardIndex] && (
        <View style={styles.cardInfo}>
          <Text style={styles.priceText}>Tarification: {data.results[currentCardIndex].price_type}</Text>
          <View style={styles.cardInfoRow}>
            <View style={styles.iconBackground}>
              <Icon name="calendar" size={20} color="black" style={styles.icon} />
            </View>
            <Text style={styles.cardInfoText}>{formatDate(data.results[currentCardIndex].occurrences, "<br />")}</Text>
          </View>
          <View style={styles.cardInfoRow}>
            <View style={styles.iconBackground}>

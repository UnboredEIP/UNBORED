...
` && console.error(
    ...
    events && !asChild && Object.assign(events, {
      ...
      minPressDuration: 0
    }), process.env.NODE_ENV === "development" && time && time`events`, process.env.NODE_ENV === "development" && debugProp === "verbose" && console.info("events", { events, isHoverable, attachPress }), hooks.useEvents?.(viewProps, events, splitStyles, setStateShallow, staticConfig);
    ...
    process.env.NODE_ENV !== "test" && (elementType === BaseText || elementType === BaseView) ? process.env.TAMAGUI_OPTIMIZE_NATIVE_VIEWS ? (viewProps.children = content, content = createElement(
      ...
      viewProps
    )) : (viewProps.children = content, content = elementType.render(viewProps, viewProps.ref)) : content = createElement(elementType, viewProps, content), hooks.useChildren && (content = hooks.useChildren?.(content, viewProps, events, staticConfig)), process.env.NODE_ENV === "development" && time && time`create-element`;
